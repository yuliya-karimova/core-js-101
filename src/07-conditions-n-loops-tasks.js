...
function isCreditCardNumber(ccn) {
  const reversedCcn = ccn.toString().split('').reverse().map(Number);

  const sum = reversedCcn.reduce((acc, curNum, i) => {
    if (i % 2 === 0) {
      return acc + curNum;
    } return acc + (curNum * 2 > 9 ? curNum * 2 - 9 : curNum * 2);
  }, 0);
...
function getCommonDirectoryPath(pathes) {
  const result = [];
  const splitedPathes = pathes.sort().map((path) => path.split('/'));
  splitedPathes[0].forEach((part, i) => {
    if (splitedPathes.slice(1).every((path) => path[i] === part)) result.push(`${part}/`);
  });
  return result.join('');
}
